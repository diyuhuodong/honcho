"""Sync HTTP client for Honcho SDK."""
...
class HonchoHTTPClient:
    """Sync HTTP client for the Honcho API with retry logic and timeout support."""
        ...
        while attempt <= self.max_retries:
            try:
                ...
                if (
                    ...
                    last_error = error
                    time.sleep(self._get_retry_delay(attempt, retry_after))
                    attempt += 1
                    continue
    ...
    def get(
        ...
        *,
        ...
        """Make a GET request."""
        return self.request("GET", path, query=query, headers=headers, timeout=timeout)
